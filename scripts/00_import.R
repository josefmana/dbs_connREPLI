...
# read outcome data
d0 <- read.csv( here("_raw","redcap_export_20231102.csv"), sep = "," ) # outcome data
...
d3 <-
  ...
  list(
    fMRI_1 = read.csv( here("_raw","overlaps_202304_1.csv"), sep = ","), # the first set of overlaps (Clemens, April 2023)
    fMRI_2 = read.csv( here("_raw","overlaps_202304_2.csv"), sep = ","), # the first set of overlaps (Clemens, April 2023)
    mulvar = read.csv( here("_raw","overlaps_202306.csv"), sep = ";", dec = "," ), # the second set of overlaps (Garance, June 2023)
    multip = read.csv( here("_raw","overlaps_202310.csv"), na.strings = "NaN", skip = 1 ) # the third set of overlaps (Patricia & Nanditha, October 2023)
  )
...
saveRDS( object = d1, file = here("_data","response_data.rds") )
...
df <-
  ...
  lapply(
    ...
    function(i)
      sapply( c("pre","r1"), function(j) colSums( d1[[i]][ ,j, ], na.rm = F ) )
...
# prepare a data frame for manual control/fill-in
d2 <-
  
  df[ df$id %in% ids, ] %>%
  
  # keep UPDRS-III data only
  select( id, starts_with("updrs_iii") ) %>%
  
  # prepare columns for manual fill-in
  mutate(
    pre_med_off_source = ifelse( is.na(updrs_iii_none_off_pre), NA, "items"),
    pre_med_on_source = ifelse( is.na(updrs_iii_none_on_pre), NA, "items"),
    post_stim_off_source = ifelse( is.na(updrs_iii_off_off_r1), NA, "items"),
    post_stim_on_source = ifelse( is.na(updrs_iii_on_off_r1), NA, "items"),
    post_measured_first = NA
  )

# save it
write.table( d2, here("_data","updrsiii_data.csv"), sep = ",", row.names = F, quote = F )
...
o <-
  ...
  data.frame(
    ...
    response = c(
      "drsii", "bdi", rep("updrs_iii,mds_updrs_iii",3), # fMRI overlaps (Clemens)
...
# save it
write.table( x = o, file = here("_data","respred_pairs.csv"), sep = ";", na = "NA", row.names = F, quote = F )
...
write.table( x = df, file = here("_data","combined_respred.csv"), sep = ",", na = "NA", dec = ".", row.names = F, quote = F )
